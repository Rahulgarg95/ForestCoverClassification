    ...
    def valuesFromSchema(self):
        """
                        ...
                        On Failure: Raise ValueError,KeyError,Exception
                                ...
                                """
        try:
            if not self.dbObj.isCollectionPresent('forestCoverDB','train_schema'):
                with open(self.schema_path, 'r') as f:
                    dic = json.load(f)
                    f.close()
                self.dbObj.insertOneRecord('forestCoverDB','train_schema',dic)
            dic = self.dbObj.getRecords('forestCoverDB', 'train_schema')
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[2]) == LengthOfDateStampInFile:
                        if len(splitAtDot[3]) == LengthOfTimeStampInFile:
                            self.azureObj.copyFileToFolder(self.Batch_Directory,'Training_Good_Raw_Files_Validated',filename)
        ...
        try:
            ...
            for file in file_list:
                ...
                if csv.shape[1] == NumberofColumns:
                    ...
                    self.azureObj.moveFileToFolder('Training_Good_Raw_Files_Validated','Training_Bad_Raw_Files_Validated',file)
